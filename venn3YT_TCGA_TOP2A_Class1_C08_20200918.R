...
library("data.table")
SARC_SC08 <- SARC[SARC$Spearman.s.Correlation >= SCvalueP,]
...
LGG_SC08 <- LGG[LGG$Spearman.s.Correlation >= SCvalueP,]
...
KIRP_SC08 <- KIRP[KIRP$Spearman.s.Correlation >= SCvalueP,]
...
KIRC_SC08 <- KIRC[KIRC$Spearman.s.Correlation >= SCvalueP,]
...
LIHC_SC08 <- LIHC[LIHC$Spearman.s.Correlation >= SCvalueP,]
...
venn.diagram(x = list(A, B, C, D, E) ,
             ...
             filename = 'Venn20200917Class1SC008V1.png',
             output=TRUE,
             imagetype="png", 
             scaled = FALSE,
             col = "Gray",
...
library("png")
pp <- readPNG("Venn20200917Class1SC008V1.png")
plot.new() 
